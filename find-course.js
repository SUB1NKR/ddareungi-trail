window.addEventListener("DOMContentLoaded", () => {
  ...
  function openExternalNotice(url){if(!url)return;pendingExternalUrl=url;clearTimeout(externalNoticeTimer);document.body.classList.add("is-modal-open");if(!externalNotice){externalNoticeTimer=setTimeout(()=>window.location.assign(url),2000);return}externalNotice.style.display="flex";externalNotice.setAttribute("aria-hidden","false");requestAnimationFrame(()=>externalNotice.classList.add("is-visible"));externalNoticeTimer=setTimeout(()=>{if(!pendingExternalUrl)return;const targetUrl=pendingExternalUrl;pendingExternalUrl="";window.location.assign(targetUrl)},2000)}
  function closeExternalNotice(){clearTimeout(externalNoticeTimer);pendingExternalUrl="";document.body.classList.remove("is-modal-open");externalNotice?.classList.remove("is-visible");externalNotice?.setAttribute("aria-hidden","true");if(externalNotice){setTimeout(()=>{if(!externalNotice.classList.contains("is-visible"))externalNotice.style.display="none"},280)}}
