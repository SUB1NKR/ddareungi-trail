window.addEventListener("DOMContentLoaded", () => {
  ...
  function openExternalNotice(url){pendingExternalUrl=url;clearTimeout(externalNoticeTimer);document.body.classList.add("is-modal-open");externalNotice?.classList.add("is-visible");externalNotice?.setAttribute("aria-hidden","false");externalNoticeTimer=setTimeout(()=>{if(!pendingExternalUrl)return;const targetUrl=pendingExternalUrl;pendingExternalUrl="";window.location.href=targetUrl},2000)}
  function closeExternalNotice(){clearTimeout(externalNoticeTimer);pendingExternalUrl="";document.body.classList.remove("is-modal-open");externalNotice?.classList.remove("is-visible");externalNotice?.setAttribute("aria-hidden","true")}
